...
dredge_FRic <- dredge(FRic) %>%
  filter(AICc == min(AICc)) %>% 
  mutate(CWM = "FRic") %>%
  select(CWM, climate, treatment, "climate:treatment", AICc)

dredge_FEve <- dredge(FEve) %>%
  filter(AICc == min(AICc)) %>% 
  mutate(CWM = "FEve") %>%
  select(CWM, AICc)

dredge_FDiv <- dredge(FDiv) %>%
  filter(AICc == min(AICc)) %>% 
  mutate(CWM = "FDiv") %>%
  select(CWM, climate, treatment, "climate:treatment", AICc)

dredge_FDis <- dredge(FDis) %>%
  filter(AICc == min(AICc)) %>% 
  mutate(CWM = "FDis") %>%
  select(CWM, climate, treatment, "climate:treatment", AICc)
...
pretty_table <- dredge_table %>% 
  ...
  gt() %>% 
  tab_header(
    title = "FD Best-Fit Models & AIC Values"
  ) %>% 
  tab_spanner(
...
# gtsave(data = pretty_table, filename = "FD_AIC_table.png", path = "~/github/Tejon_Functional_Traits/Figures")
