...
# interaction model:
LA_CWM_3 <- lm(data = cwm_traits_adj, log_la ~ climate * treatment, na.action = na.fail)
...
# effect size? not sure which method is the best
# eta_squared(n_aov)
...
simulateResiduals(LDMC_CWM_1, plot= T) #looks good
shapiro.test(residuals(LDMC_CWM_1)) #normal
anova(LDMC_CWM_1)
...
simulateResiduals(seed_mass_CWM_1, plot= T) #looks good
shapiro.test(residuals(seed_mass_CWM_1)) #normal
anova(seed_mass_CWM_1)
...
shapiro.test(residuals(n_CWM_1)) 
qqPlot(residuals(n_CWM_1))
anova(n_CWM_1)
