...
```{r}
ldmc <- graphs(model_name = LDMC_CWM,
       model_terms = c("treatment", "climate"),
...
### e. Seed Mass
```{r}
seed_mass <- graphs(model_name = log_seed_mass,
       model_terms = c("treatment", "climate"),
...
# no significant difference, so doing manually

predict <- ggpredict(log_n_CWM, terms = c("treatment", "climate"))
test <- as.data.frame(predict)

anno <- as.data.frame(summary(emmeans(log_n_CWM, specs = pairwise ~ climate + treatment))$contrasts) %>%
  filter(p.value <= 0.05) %>%
  separate(col = contrast, into = c("plot_1", "plot_2"), sep = " - ") %>%
  separate(col = plot_1, into = c("plot_1_climate", "plot_1_treatment"), sep = ",") %>%
  separate(col = plot_2, into = c("plot_2_climate", "plot_2_treatment"), sep = ",") %>%
