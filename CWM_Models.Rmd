...
test <- emmeans(LA_CWM, ~ climate | treatment)
pwpp(test) # difficult to interpret
...
# multcomp::cld(pairs(emmeans(LA_CWM, ~ climate | treatment)))

# test <- emmeans(LA_CWM, pairwise ~ climate | treatment)

data <- as.data.frame(test$contrasts) %>% 
...
ggplot(data, aes(x = contrast, y = estimate)) +
  geom_boxplot() +
...
plot(test, 
     ...
     CI = FALSE,
     comparisons = TRUE, # If arrows overlap, they are not statistically significant
     colors = c("black", "black", "blue", arrow_colors(climate))) + # color names for estimates, CIs, PIs, and comparison arrows, respectively
  # this is a ggplot so you can customize as normal
  theme_bw() +   
  labs(x = "Estimated marginal mean (LA CWM)", y = "Climate")
...
pretty_table <- dredge_table %>% 
  ...
  gt() %>% 
  tab_header(
    title = "CWM Best-Fit Models & AIC Values"
  ) %>% 
  tab_spanner(
  ...
  tab_source_note(
    source_note = "+ indicates selection of fixed effect term for best-fit model"
  )
...
emm_plot <- function(model, Climate = TRUE, Treatment = TRUE) {
  if (Climate == TRUE && Treatment == TRUE) return({
    plot(emmeans(model, ~ climate | treatment), 
         CIs = FALSE, 
         type = "response", 
         comparisons = TRUE, 
         colors = c("black", "black", "blue", arrow_colors(treatment))) + # color names for estimates, CIs, PIs, and comparison arrows, respectively
      theme_bw() +
      labs(x = "Estimated marginal mean", y = "Climate")
    })
  if (Climate == TRUE && Treatment == FALSE) return({
    plot(emmeans(LA_CWM, ~ climate), type = "response", comparisons = TRUE, colors = c("black", "black", "blue", "black")) + 
      theme_bw() +
      labs(x = "Estimated marginal mean", y = "Climate")
    })
  if (Climate == FALSE && Treatment == TRUE) return({
    plot(emmeans(LA_CWM, ~ treatment), type = "response", comparisons = TRUE, colors = c("black", "black", "blue", "black")) + 
      theme_bw() +
...
ggsave("CWM_emmeans.png", plot = CWM_emmeans_all, path = "~/github/Tejon_Functional_Traits/Figures", height = 10, width = 10)
