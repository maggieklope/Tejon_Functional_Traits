...
emmeans(LA_CWM, pairwise ~ climate | treatment)
emmeans(LA_CWM, pairwise ~ cliamte | treatment)
...
#### b. SLA CWM
- Averaging for treatment, climate significanlty different between Arid-Int and Arid-Mesic
- Averaging for climate, treatment significantly different between Open-Total and Partial-Total, but not Open-Partial
...
# Checking data
simulateResiduals(LA_CWM, plot= T) # looks good
shapiro.test(residuals(LA_CWM)) # normal
...
emmeans(LA_CWM, pairwise ~ treatment)
emmeans(LA_CWM, pairwise ~ climate * treatment)
LA_pairs_1 <- emmeans(LA_CWM, pairwise ~ climate | treatment)
LA_pairs_2 <- emmeans(LA_CWM, pairwise ~ treatment | climate)

plot(emmeans(LA_CWM, pairwise ~ climate * treatment))

as.data.frame(LA_pairs_1$contrasts)
...
- I started setting this up to look at changes in weighted means when grouped by funcitonal group, but I think it would be more interesting to use the species-level, unweighted data.
...
grass_glm <- glmmTMB(data = grass, weighted_la ~ climate * treatment, family = gaussian)
...
```{r}
forb_glm <- glmmTMB(data = forb, weighted_la ~ climate * treatment, family = gaussian)
dredge(forb_glm) # NEED TO DO MODEL AVERAGING
forb_glm <- glmmTMB(data = forb, weighted_la ~ treatment, family = gaussian)
...
```{r}
shrub_glm <- glmmTMB(data = shrub, weighted_la ~ climate * treatment, family = gaussian)
dredge(shrub_glm) # best model: trait ~ climate
shrub_glm  <- glmmTMB(data = shrub, weighted_la ~ climate, family = gaussian)
