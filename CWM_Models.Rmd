...
plot2 <- emm_plot(sla_emmeans)+
  ...
  geom_signif(annotation = "*",
    y_position = 3.25, xmin = 1, xmax = 2,
    tip_length = c(.02, .4)) +
  # Arid-Open - Arid-Total
  geom_signif(annotation = "***",
    y_position = 4.25, xmin = 1, xmax = 3,
    tip_length = c(.3, .05)) +
  # Arid-Partial - Arid-Total
   geom_signif(annotation = "***",
    y_position = 4.75, xmin = 2, xmax = 3,
    tip_length = c(.05, .1))
...
emmeans(log_n_CWM, pairwise ~ treatment | climate)

plot4 <- emm_plot(n_emmeans) +
  # adding title
  labs(title = "Leaf Nitrogen Concentration (log) CWM") +
   # adding significance of comparisons
  # Arid-Partial - Arid-Total
  geom_signif(annotation = "*",
    y_position = 1.15, xmin = 2, xmax = 3,
    tip_length = c(.3, .025)) +
  # Int-Partial - Int-Total
  geom_signif(annotation = "*",
    y_position = 1.25, xmin = 5, xmax = 6,
    tip_length = c(.35, .05)) +
  # Mesic-Partial - Mesic-Total
   geom_signif(annotation = "*",
    y_position = 1.05, xmin = 8, xmax = 9,
    tip_length = c(.3, .05))

# plot 4
  
ggsave("leaf_n_CWM_emmean.png", plot = plot4, path = "~/github/Tejon_Functional_Traits/Figures", height = 4, width = 5, bg = "transparent")
```
