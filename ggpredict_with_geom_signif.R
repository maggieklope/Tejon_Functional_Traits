...
graphs <- function(model_name, model_terms, model_formula, plot_title, y_label, x_label, y_positions = 20) {
  ...
  predict <- as.data.frame(ggpredict(model_name, terms = model_terms))
...
LDMC_CWM_predict <- ggpredict(LDMC_CWM, terms = c("treatment", "climate"))
test <- as.data.frame(LDMC_CWM_predict)
...
anno <- as.data.frame(summary(emmeans(SLA_CWM, specs = pairwise ~ climate * treatment))$contrasts) %>% 
  filter(p.value <= 0.05) %>% 
  separate(col = contrast, into = c("plot_1", "plot_2"), sep = " - ") %>% 
  separate(col = plot_1, into = c("plot_1_climate", "plot_1_treatment"), sep = ",") %>% 
  separate(col = plot_2, into = c("plot_2_climate", "plot_2_treatment"), sep = ",") %>% 
...
graphs(model_name = LDMC_CWM, model_terms = c("treatment", "climate"), model_formula = "pairwise ~ climate * treatment", plot_title = "Title", y_label = "label", x_label = "label", y_positions = c(50, 60, 45))
