...
seed_mass_plot_1 <- plot(funct_seed_mass_emm, type = "response", by = NULL, CIs = FALSE, comparisons = TRUE, horizontal = FALSE, colors = "darkgreen") +
...
# plot
seed_mass_plot_2 <- plot(funct_seed_mass.rg, type = "response", by = NULL, CIs = FALSE, comparisons = TRUE, horizontal = FALSE, colors = "darkgreen") +
  facet_grid(. ~ functional_group, scales = "free", space = "free") +
   geom_segment(aes_(y = ~the.emmean, yend = ~lcmpl, x = ~pri.fac, xend = ~pri.fac), 
               arrow = arrow(length = ggplot2::unit(.07, "inches"), type = "closed"),
               color ="black") +
...
###### Leaf Area
```{r}
la_emmeans <- emmeans(funct_la, pairwise ~ treatment + climate + functional_group)
