...
for_FD <- read_csv("~/github/Tejon_Functional_Traits/Datasheets/cwm_traits_updated.csv") %>% 
...
dredge(FEve) #best model includes only the intercept
...
dat <- ggpredict(FDiv, terms = c("treatment", "climate"))
plot(dat, facet = TRUE)
```
...
# estimated marginal means plot
dat <- ggpredict(FDis, terms = c("treatment", "climate"))
plot(dat, facet = TRUE)
