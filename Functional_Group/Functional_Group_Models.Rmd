...
##### 1. leaf area
```{r, warning=FALSE}
funct_la <- glmmTMB(data = cwm_functional, weighted_la ~ climate * treatment * functional_group + (1|Block), family = gaussian, na.action = na.fail) # AIC 620.6
funct_la <- glmmTMB(data = cwm_functional, weighted_la ~ climate * functional_group + treatment * functional_group + (1|Block), family = gaussian, na.action = na.fail) # AIC 633.1
dredge(funct_la)
...
funct_la <- glmmTMB(data = cwm_functional, log_la ~ climate *  functional_group + treatment * functional_group + (1|Block), family = gaussian, na.action = na.fail)
dredge(funct_la)
funct_la <- glmmTMB(data = cwm_functional, log_la ~ climate + treatment + functional_group + climate:functional_group + functional_group:treatment + (1|Block), family = gaussian, na.action = na.fail)

simulateResiduals(funct_la, plot= T) #looks good
shapiro.test(residuals(funct_la)) # but residuals are not normal - is this ok?
...
funct_sla <- glmmTMB(data = cwm_functional, weighted_sla ~ climate * functional_group + treatment * functional_group + (1|Block), family = gaussian, na.action = na.fail)
dredge(funct_sla)
...
# trying log transformation with gaussian
funct_sla <- glmmTMB(data = cwm_functional, log_sla ~ climate * functional_group + treatment * functional_group + (1|Block), family = gaussian, na.action = na.fail)
dredge(funct_sla)
...
#   dplyr::select(!c(df, logLik, AICc, delta, weight, "cond((Int))", "disp((Int))"))
#   
...
#   mutate(`Climate:Functional Group` = str_replace(`cond(climate:functional_group)`, "\\+","x")) %>% 
#   dplyr::select(CWM, Climate, "Functional Group", Treatment, "Climate:Functional Group", "Functional Group:Treatment", "Climate:Functional Group")
# 
# dredge_table
# 
