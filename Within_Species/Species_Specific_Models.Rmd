...
ggplot(data = traits, aes(x = indiv_area, color = treatment)) +
  geom_histogram()
...
simulateResiduals(model_1, plot= T) # meh
shapiro.test(residuals(model_1)) # not normal

# LDMC
model_1 <- glmmTMB(data = traits, ldmc ~ treatment * climate + species + (1|new_block), family = gaussian, na.action = na.fail)
dredge(model_1) 
model_1 <- glmmTMB(data = traits, sla ~ species + (1|new_block), family = gaussian, na.action = na.fail)

simulateResiduals(model_1, plot= T) # ok
shapiro.test(residuals(model_1)) # not normal
...
dredge(bro_hor_ldmc) # best fit model is intercept
bro_hor_ldmc <- glmmTMB(data = bro_hor, ldmc ~ 1 + (1|new_block), family = gaussian, na.action = na.fail)
...
shapiro.test(residuals(gal_apa_sla)) # not normal
```
#### LDMC
```{r}
...
pla_not_ldmc <- glmmTMB(data = pla_not, ldmc ~ 1 + (1|new_block), family = gaussian, na.action = na.fail)
...
dredge(ribes_ldmc) # best fit intercept only
