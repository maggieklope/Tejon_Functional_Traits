...
p <- plot(funct_la_emm, by = NULL, CIs = FALSE, comparisons = TRUE, horizontal = FALSE, colors = "black") +
  ...
  labs(title = "Leaf Area CWM by Functional Group", x = element_blank())
...
p <- plot(funct_sla_emm, by = NULL, CIs = FALSE, comparisons = TRUE, horizontal = FALSE, colors = "black") +
  ...
  theme(panel.spacing = unit(0, "lines"),
        panel.border = element_rect(color = "black", fill = NA, size = 0.5),
        axis.text.x = element_text(angle = 45, vjust = 0.5, hjust=1)) +
  labs(title = "Specific Leaf Area CWM by Functional Group", x = element_blank())
